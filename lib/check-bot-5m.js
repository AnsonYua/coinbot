...
export function summarize5mOutcomes(decisions) {
  const wins = decisions.filter((item) => item.outcome_status === "win").length;
  const losses = decisions.filter((item) => item.outcome_status === "loss").length;
  const unresolved = decisions.filter((item) => item.outcome_status == null || item.outcome_status === "unresolved").length;
