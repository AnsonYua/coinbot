...
export function summarize5mOutcomes(decisions) {
  ...
  const totalStakeUsd = decisions.reduce((sum, item) => sum + stakeUsdForDecision(item), 0);
  ...
  return {
    ...
    totalStakeUsd: roundMoney(totalStakeUsd),
...
export function build5mOutcomeSummaryText({ summary, settled }) {
  const lines = [
    `<b>BTC 5m results today</b>`,
    `realized_pnl_usd: ${formatMoney(summary.realizedPnlUsd)}`,
    `profit_usd: ${formatMoney(summary.profitUsd)}`,
    `loss_usd: ${formatMoney(summary.lossUsd)}`,
    `unresolved_stake_usd: ${formatMoney(summary.unresolvedStakeUsd)}`,
    `total_stake_usd: ${formatMoney(summary.totalStakeUsd)}`,
    `wins: ${summary.wins}`,
    `losses: ${summary.losses}`,
    `unresolved: ${summary.unresolved}`,
    `win_rate: ${summary.winRate == null ? "n/a" : `${(summary.winRate * 100).toFixed(2)}%`}`,
  ];
  for (const item of settled.slice(0, 5)) {
    lines.push(`settled: ${item.marketSlug} ${item.side} -> ${item.outcomeStatus}`);
  }
  return lines.join("\n");
}
...
async function settlePrevious5mTrades({
  ...
  return {
    settled,
    summary: summarize5mOutcomes(todaysTrades),
  };
}
