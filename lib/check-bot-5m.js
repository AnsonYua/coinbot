...
import { fetch5mTriggerFeatures } from "./binance.js";
import {
  active5mMarketSlug,
...
const MAX_ENTRY_PRICE_INCLUSIVE = 0.92;
const MIN_ALIGNED_BTC_DISTANCE = 0.00025;
const MAX_ALIGNED_BTC_DISTANCE = 0.0007;
const MIN_ALIGNED_BTC_MOMENTUM_60 = -0.0002;
...
export function is5mEntryPriceEligible(price) {
  return Number.isFinite(price) && price > MIN_ENTRY_PRICE_EXCLUSIVE && price <= MAX_ENTRY_PRICE_INCLUSIVE;
}

export function evaluate5mEntryPrice(price, side, btcFeatures) {
  return build5mEvaluation(price, side, btcFeatures);
}
...
function alignedValue(side, value) {
  if (!Number.isFinite(value)) return null;
  return side === "YES" ? value : -value;
}

function build5mEvaluation(price, side, btcFeatures) {
  const alignedDistance = alignedValue(side, btcFeatures?.btcDistance);
  const alignedMomentum60 = alignedValue(side, btcFeatures?.btcMomentum60);
  const priceEligible = is5mEntryPriceEligible(price);
  const hasBtcFeatures = alignedDistance != null && alignedMomentum60 != null;
  const distanceEligible =
    hasBtcFeatures &&
    alignedDistance >= MIN_ALIGNED_BTC_DISTANCE &&
    alignedDistance <= MAX_ALIGNED_BTC_DISTANCE;
  const momentumEligible = hasBtcFeatures && alignedMomentum60 >= MIN_ALIGNED_BTC_MOMENTUM_60;
  let reason = "balanced_rule_passed";
  if (price == null) {
    reason = "missing_buy_price";
  } else if (!priceEligible) {
    reason = "price_outside_band";
  } else if (!hasBtcFeatures) {
    reason = "missing_btc_features";
  } else if (!distanceEligible) {
    reason = "btc_distance_outside_band";
  } else if (!momentumEligible) {
    reason = "btc_momentum_too_negative";
  }
  return {
    passes: priceEligible && distanceEligible && momentumEligible,
    reason,
    minExclusive: MIN_ENTRY_PRICE_EXCLUSIVE,
    maxInclusive: MAX_ENTRY_PRICE_INCLUSIVE,
    minAlignedBtcDistance: MIN_ALIGNED_BTC_DISTANCE,
    maxAlignedBtcDistance: MAX_ALIGNED_BTC_DISTANCE,
    minAlignedBtcMomentum60: MIN_ALIGNED_BTC_MOMENTUM_60,
    alignedBtcDistance: alignedDistance,
    alignedBtcMomentum60: alignedMomentum60,
  };
}
...
export function buildSignalText(summary) {
  return [
    ...
    `price band: &gt;${MIN_ENTRY_PRICE_EXCLUSIVE} and &lt;=${MAX_ENTRY_PRICE_INCLUSIVE}`,
    ...
    `aligned distance band: ${formatMaybeNumber(MIN_ALIGNED_BTC_DISTANCE * 100, 3)}% to ${formatMaybeNumber(MAX_ALIGNED_BTC_DISTANCE * 100, 3)}%`,
    `min aligned momentum 60s: ${formatMaybeNumber(MIN_ALIGNED_BTC_MOMENTUM_60 * 100, 3)}%`,
...
function buildSummary({ market, triggerUtc, dryRun, yesPrice, noPrice, btcFeatures, btcFeatureError }) {
  const yesEvaluation = build5mEvaluation(yesPrice, "YES", btcFeatures);
  const noEvaluation = build5mEvaluation(noPrice, "NO", btcFeatures);
  return {
    ok: true,
    ...
    yes: {
      ...
      alignedBtcDistance: yesEvaluation.alignedBtcDistance,
      alignedBtcMomentum60: yesEvaluation.alignedBtcMomentum60,
    },
    no: {
      ...
      alignedBtcDistance: noEvaluation.alignedBtcDistance,
      alignedBtcMomentum60: noEvaluation.alignedBtcMomentum60,
    },
    actions: [],
  };
}
...
function buildDecisionDoc({
  ...
  return {
    ...
    model_source: "price_distance_momentum",
    ...
    aligned_btc_distance: evaluation.alignedBtcDistance ?? null,
    aligned_btc_momentum_60: evaluation.alignedBtcMomentum60 ?? null,
    min_aligned_btc_distance: MIN_ALIGNED_BTC_DISTANCE,
    max_aligned_btc_distance: MAX_ALIGNED_BTC_DISTANCE,
    min_aligned_btc_momentum_60: MIN_ALIGNED_BTC_MOMENTUM_60,
    max_entry_price: MAX_ENTRY_PRICE_INCLUSIVE,
...
async function process5mSide({
  ...
  const evaluation = build5mEvaluation(entryPrice, side, btcFeatures);
  ...
  try {
    ...
    if (!liquidity.canFullyFill) {
      throw new Error(
        ...
        `${formatMaybeNumber(liquidity.availableShares, 3)} available, need ${TRADE_SHARES}`,
      );
    }
...
export async function runCheck5m({ dryRunOverride, deps = {} } = {}) {
  const {
    ...
    fetch5mTriggerFeaturesImpl = fetch5mTriggerFeatures,
  ...
  try {
    btcFeatures = await fetch5mTriggerFeaturesImpl(market.startTs);
  } catch (error) {
    btcFeatureError = error instanceof Error ? error.message : String(error);
    await insertErrorLogImpl({
      created_at: now,
      source: "binance_features_5m",
      market_slug: market.slug,
