...
function buySizeFromStake({ stakeUsd, price }) {
  if (!Number.isFinite(stakeUsd) || !Number.isFinite(price) || price <= 0 || price >= 1) {
    return 0;
  }
  return round6(stakeUsd / price);
}

export async function placeBuyOrder({ tokenId, entryPrice, stakeUsd, market }) {
  const client = await getTradingClient();
  const price = Number(entryPrice.toFixed(3));
  const size = buySizeFromStake({ stakeUsd, price });
