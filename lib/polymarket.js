...
export async function fetch5mMarketByStartTs(marketStartTs) {
  return fetchMarketBySlug(`btc-updown-5m-${marketStartTs}`);
}
...
export async function fetchGammaMarketBySlug(slug) {
  return fetchJson(`${GAMMA_BASE}/markets/slug/${encodeURIComponent(slug)}`);
}

export function normalizeGammaMarket(market) {
  const [yesTokenId, noTokenId] = JSON.parse(market.clobTokenIds);
  const startTs = Number(market.slug.split("-").at(-1));
  const intervalSeconds = market.slug.includes("-5m-") ? 300 : 900;
  return {
    slug: market.slug,
    startTs,
    endTs: startTs + intervalSeconds,
...
export function resolveWinningSideFromGammaMarket(market) {
  ...
  const noPrice = Number(outcomePrices[1]);
  if (!Number.isFinite(yesPrice) || !Number.isFinite(noPrice)) return null;
  if (yesPrice === 1 && noPrice === 0) return "YES";
  if (yesPrice === 0 && noPrice === 1) return "NO";
  return null;
}
