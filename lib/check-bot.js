...
function buildSignalText(summary) {
  return [
    `<b>BTC 15m volume model check</b>`,
    `market: ${summary.marketSlug}`,
    `trigger: ${summary.triggerUtc}`,
    `question: ${summary.question}`,
    `buy price band: ${summary.priceBand.min} to ${summary.priceBand.max}`,
    ...
    `trigger vol ratio 1m: ${summary.features.triggerVolumeRatio1m}`,
...
function buildSummary({ config, market, triggerUtc, dryRun, features, yesPrice, noPrice, yesEvaluation, noEvaluation }) {
  return {
    ok: true,
    ...
    dryRun,
    priceBand: {
      min: config.minBuyPrice,
      max: config.maxBuyPrice,
    },
    features: {
      ...
      triggerVolumeRatio1m: Number(features.triggerVolumeRatio1m.toFixed(4)),
    },
    yes: {
      tokenId: market.yesTokenId,
      price: yesPrice,
