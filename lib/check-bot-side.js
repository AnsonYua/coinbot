...
function buildActionText({ marketSlug, side, price, dryRun, evaluation }) {
  const maxPrice = evaluation?.probability != null && evaluation?.minEdge != null
    ? evaluation.probability - evaluation.minEdge
    : null;
  const targetBudget = maxPrice != null ? TRADE_SHARES * maxPrice : null;
  return [
    `<b>BTC 15m volume model BUY ${side}</b>`,
    `market: ${marketSlug}`,
    `decision_price: ${price}`,
    `max_buy_price: ${formatMaybeNumber(maxPrice)}`,
    `target_shares: ${TRADE_SHARES}`,
    `target_budget_usd: ${formatMaybeNumber(targetBudget, 3)}`,
...
function buildDecisionDoc({
  ...
  return {
    ...
    trigger_volume_ratio_1m: features.triggerVolumeRatio1m ?? null,
    ...
    model_min_probability: evaluation?.minProbability ?? null,
...
async function handleLiveBuy({
  ...
  try {
    ...
    if (!liquidity.canFullyFill) {
      throw new Error(
        `not enough ask liquidity at or below max buy price ${formatMaybeNumber(action.maxPrice)}: ` +
    ...
    const executionDelivery = await safeTelegram(() => sendActionMessage(config, [
      `<b>BTC 15m volume model ${headline}</b>`,
      `market: ${market.slug}`,
      `decision_price: ${entryPrice}`,
      `max_buy_price: ${formatMaybeNumber(action.maxPrice)}`,
      `best_ask: ${formatMaybeNumber(action.bestAsk)}`,
      `target_shares: ${TRADE_SHARES}`,
      `target_budget_usd: ${formatMaybeNumber(action.maxPrice * TRADE_SHARES, 3)}`,
    ...
    const failureDelivery = await safeTelegram(() => sendActionMessage(
      config,
      `<b>BTC 15m volume model BUY ${side} failed</b>\nmarket: ${escapeTelegramHtml(market.slug)}\ndecision_price: ${escapeTelegramHtml(String(entryPrice))}\n${escapeTelegramHtml(message)}`,
    ));
    await recordExecutionMessage({
      marketSlug: market.slug,
      side,
