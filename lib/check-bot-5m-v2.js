...
import {
  build5mTaFeatures,
  evaluate5mOptimizedCandlePattern,
  evaluate5mTaSide,
  load5mTaProbabilityMap,
  TA_5M_CANDLE_MAX_ENTRY_PRICE,
  TA_5M_CANDLE_MIN_CONSERVATIVE_EDGE,
  TA_5M_CANDLE_MIN_ENTRY_PRICE,
  TA_5M_CANDLE_PATTERN_PARAMS,
  TA_5M_CANDLE_PATTERN_SOURCE,
...
function formatPatternList(patterns) {
  return Array.isArray(patterns) && patterns.length > 0 ? patterns.join(",") : "none";
}
...
export function is5mEntryPriceEligible(price) {
  return Number.isFinite(price) && price >= TA_5M_CANDLE_MIN_ENTRY_PRICE && price <= TA_5M_CANDLE_MAX_ENTRY_PRICE;
}
...
function build5mEvaluation(price, side, btcFeatures, mapPayload) {
  ...
  const baseEvaluation = evaluate5mTaSide({
    side,
    entryPrice: price,
    features: btcFeatures,
    mapPayload,
  });
  const candle = evaluate5mOptimizedCandlePattern(btcFeatures, side);
  if (baseEvaluation.passes) {
    return {
      ...baseEvaluation,
      entryRule: "base_ta_edge",
      baseReason: baseEvaluation.reason,
      candleConfirmed: candle.confirms,
      candleReason: candle.reason,
      candleMatchedPatterns: candle.matchedPatterns,
      candlePatternSource: candle.source,
      candleStats: candle,
    };
  }

  const candleEvaluation = evaluate5mTaSide({
    side,
    entryPrice: price,
    features: btcFeatures,
    mapPayload,
    minEntryPrice: TA_5M_CANDLE_MIN_ENTRY_PRICE,
    maxEntryPrice: TA_5M_CANDLE_MAX_ENTRY_PRICE,
    minConservativeEdge: TA_5M_CANDLE_MIN_CONSERVATIVE_EDGE,
    minSupport: TA_5M_MIN_SUPPORT,
  });
  if (candleEvaluation.passes && candle.confirms) {
    return {
      ...candleEvaluation,
      passes: true,
      reason: "candle_edge_passed",
      entryRule: "optimized_candle_edge",
      baseReason: baseEvaluation.reason,
  ...
  return {
    ...
    baseReason: baseEvaluation.reason,
    candleConfirmed: candle.confirms,
    candleReason: candleEvaluation.passes ? candle.reason : candleEvaluation.reason,
    candleMatchedPatterns: candle.matchedPatterns,
    candlePatternSource: candle.source,
    candleStats: candle,
    candleEvaluationReason: candleEvaluation.reason,
    candleConservativeEdge: candleEvaluation.conservativeEdge,
    candleSupportN: candleEvaluation.supportN,
  };
}
...
export function buildSignalText(summary) {
  const selected = summary.selectedTradeSide ? sideSummaryFor(summary, summary.selectedTradeSide) : null;
  return [
    `<b>BTC 5m V2.1 check</b>`,
    `market: ${summary.marketSlug}`,
    `trigger: ${summary.triggerUtc}`,
    `question: ${escapeTelegramHtml(summary.question)}`,
    `selected: ${summary.selectedTradeSide ? `${sideLabel(summary.selectedTradeSide)} (${summary.selectedTradeSide})` : "none"}`,
    `selected rule: ${selected?.entryRule ?? "none"}`,
    `selected edge: ${formatMaybeNumber(selected?.conservativeEdge, 4)}`,
    `selected support: ${selected?.supportN ?? "n/a"}`,
    `selected candle patterns: ${formatPatternList(selected?.candleMatchedPatterns)}`,
    `rule A base: price ${TA_5M_MIN_ENTRY_PRICE}-${TA_5M_MAX_ENTRY_PRICE}, edge >= ${formatMaybeNumber(TA_5M_MIN_CONSERVATIVE_EDGE, 3)}, support >= ${TA_5M_MIN_SUPPORT}`,
    `rule B candle: price ${TA_5M_CANDLE_MIN_ENTRY_PRICE}-${TA_5M_CANDLE_MAX_ENTRY_PRICE}, edge >= ${formatMaybeNumber(TA_5M_CANDLE_MIN_CONSERVATIVE_EDGE, 3)}, support >= ${TA_5M_MIN_SUPPORT}, candle confirm`,
    ...
    `yes base reason: ${summary.yes.baseReason ?? "n/a"}`,
    `yes candle reason: ${summary.yes.candleReason ?? "n/a"}`,
    `yes entry rule: ${summary.yes.entryRule ?? "none"}`,
    `yes candle: ${summary.yes.candleConfirmed} (${formatPatternList(summary.yes.candleMatchedPatterns)})`,
    ...
    `no base reason: ${summary.no.baseReason ?? "n/a"}`,
    `no candle reason: ${summary.no.candleReason ?? "n/a"}`,
    `no entry rule: ${summary.no.entryRule ?? "none"}`,
    `no candle: ${summary.no.candleConfirmed} (${formatPatternList(summary.no.candleMatchedPatterns)})`,
...
export function build5mTradeAlertText({
  ...
  return [
    `<b>${title.replace("BTC 5m V2 ", "BTC 5m V2.1 ")}</b>`,
    `market: ${marketSlug}`,
    ...
    `entry_rule: ${entryRule || "n/a"}`,
    `candle_confirmed: ${candleConfirmed ?? "n/a"}`,
...
function buildSummary({ market, triggerUtc, dryRun, yesPrice, noPrice, btcFeatures, btcFeatureError, mapPayload }) {
  ...
  return {
    ...
    yes: {
      ...
      entryRule: yesEvaluation.entryRule,
      baseReason: yesEvaluation.baseReason,
      candleConfirmed: yesEvaluation.candleConfirmed,
      candleReason: yesEvaluation.candleReason,
      candleMatchedPatterns: yesEvaluation.candleMatchedPatterns,
      candlePatternSource: yesEvaluation.candlePatternSource,
      candleConservativeEdge: yesEvaluation.candleConservativeEdge,
      candleSupportN: yesEvaluation.candleSupportN,
    },
    no: {
      ...
      entryRule: noEvaluation.entryRule,
      baseReason: noEvaluation.baseReason,
      candleConfirmed: noEvaluation.candleConfirmed,
      candleReason: noEvaluation.candleReason,
      candleMatchedPatterns: noEvaluation.candleMatchedPatterns,
      candlePatternSource: noEvaluation.candlePatternSource,
      candleConservativeEdge: noEvaluation.candleConservativeEdge,
      candleSupportN: noEvaluation.candleSupportN,
    },
    actions: [],
  };
}
...
function buildDecisionDoc({
  ...
  return {
    ...
    model_entry_rule: evaluation.entryRule ?? null,
    model_base_reason: evaluation.baseReason ?? evaluation.reason,
    model_candle_confirmed: evaluation.candleConfirmed ?? false,
    model_candle_reason: evaluation.candleReason ?? null,
    model_candle_patterns: evaluation.candleMatchedPatterns ?? [],
    model_candle_source: evaluation.candlePatternSource ?? TA_5M_CANDLE_PATTERN_SOURCE,
    model_candle_params: TA_5M_CANDLE_PATTERN_PARAMS,
    model_candle_min_entry_price: TA_5M_CANDLE_MIN_ENTRY_PRICE,
    model_candle_max_entry_price: TA_5M_CANDLE_MAX_ENTRY_PRICE,
    model_candle_min_conservative_edge: TA_5M_CANDLE_MIN_CONSERVATIVE_EDGE,
    ...
    btc_candle_green_count: evaluation.candleStats?.greenCount ?? null,
    btc_candle_red_count: evaluation.candleStats?.redCount ?? null,
    btc_candle_4m_return: evaluation.candleStats?.fourMinuteReturn ?? null,
    btc_candle_avg_body_ratio: evaluation.candleStats?.averageBodyRatio ?? null,
    btc_candle_last_body_ratio: evaluation.candleStats?.lastBodyRatio ?? null,
    btc_candle_last_upper_wick_ratio: evaluation.candleStats?.lastUpperWickRatio ?? null,
    btc_candle_last_lower_wick_ratio: evaluation.candleStats?.lastLowerWickRatio ?? null,
...
async function process5mSide({
  ...
  const baseAction = {
    ...
    sizing,
    entryRule: evaluation.entryRule,
    candleConfirmed: evaluation.candleConfirmed,
    dryRun,
    bought: false,
    accepted: false,
  };
