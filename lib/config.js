...
export function get5mTelegramConfig(config = getConfig()) {
  ...
  if (!config.action5mBotToken) missing.push("TELEGRAM_ACTION_5M_BOT_TOKEN");
  if (!config.action5mChatId) missing.push("TELEGRAM_ACTION_5M_CHAT_ID");
  if (missing.length > 0) {
  ...
  return {
    ...
    actionBotToken: config.action5mBotToken,
    actionChatId: config.action5mChatId,
  };
}
